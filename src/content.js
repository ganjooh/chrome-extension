...
(() => {
  ...
  function createSidebar() {
    ...
    // Try pinging local server to show status, and to ensure server logs something
    try {
      fetch('http://13.203.150.222:8090/health')
        .then(r => r.json())
        .then(() => updateStatus('Ready'))
        .catch(() => updateStatus('Server not reachable'));
    } catch (_) {}
